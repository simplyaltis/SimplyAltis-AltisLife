...
class playerSettings {
	...
	class controls {
		...
		*/
        ...
        class ButtonGangList2 : Life_RscButtonMenu {
			idc = 2153;
			text = "$STR_PM_WantedList";
			onButtonClick = "[] call life_fnc_wantedMenu";
			x = 0.1 + (6.25 / 19.8) + (1 / 250 / (safezoneW / safezoneH));
        ...
        class ButtonWantedAdd : Life_RscButtonMenu { // i change life_ to Life_ ( i dont know if is important) 

			idc = 9800;
			//shortcuts[] = {0x00050000 + 2};
			text = "Wanted+"; // <- name of button
			onButtonClick = "createDialog ""life_wantedadd2"";";
			x = 0.42 + (6.25 / 19.8) + (1 / 250 / (safezoneW / safezoneH)); // horizontal position
			y = 0.805; // vertical position
			w = (6.25 / 40);
			h = (1 / 25);
		};
