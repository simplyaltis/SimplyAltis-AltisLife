...
if(_action) then {
	if(life_rupees < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_rupees] call life_fnc_numberText,
			[_upgradePrice - life_rupees] call life_fnc_numberText
		];
	};
	__SUB__(life_rupees,_upgradePrice);
