...
if(!isServer && (!isNil "life_bondlevel" OR !isNil "life_coplevel" OR !isNil "life_donator")) exitWith {
    [[profileName,getPlayerUID player,"VariablesAlreadySet"],"SPY_fnc_cookieJar",false,false] spawn life_fnc_MP;
    [[profileName,format["Variables set before client initialization...\nlife_bondlevel: %1\nlife_coplevel: %2\nlife_donator: %3",life_bondlevel,life_coplevel,life_donator]],"SPY_fnc_notifyAdmins",true,false] spawn life_fnc_MP;
    sleep 0.9;
    ["SpyGlass",false,false] execVM "\a3\functions_f\Misc\fn_endMission.sqf";
};

//Parse basic player information.
life_dosh = parseNumber (_this select 2);
life_rupees = parseNumber (_this select 3);
__CONST__(life_bondlevel,parseNumber(_this select 4));
...
switch(playerSide) do {
	...
	case civilian: {
		...
		if(count civ_position != 3 || isNil "civ_position") then { life_is_alive = false; };
	};
