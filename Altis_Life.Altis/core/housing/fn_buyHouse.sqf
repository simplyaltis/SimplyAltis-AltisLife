...
/*
	...
	Buys the house?
*/
private["_house","_uid","_action","_houseCfg","_limit"];
...
if(count life_houses >= _limit) exitWith {hint format[localize "STR_House_Max_House",__GETC__(life_houseLimit)]};
...
if(_action) then {
    if(life_rupees < (_houseCfg select 0)) exitWith {hint localize "STR_House_NotEnough"};
	...
	life_rupees = life_rupees - (_houseCfg select 0);
