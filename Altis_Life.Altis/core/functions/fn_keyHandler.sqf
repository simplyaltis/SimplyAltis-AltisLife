...
switch (_code) do
{
	...
	case 19:
	{
        ...
        if(license_civ_bountyhunter && uniform player == "U_B_CombatUniform_mcam_worn") then
        {
        ...
        {
            if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,west]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && !(player getVariable "restrained") && speed cursorTarget < 1 && animationState cursorTarget == "Incapacitated") then
            {
                if([false,"zipties",1] call life_fnc_handleInv) then
                {
                    [] call life_fnc_restrainAction;
	...
	{
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 3.5 && speed cursorTarget < 1) then
		{
	...
	case 22:
	{
		if(!_alt && !_ctrlKey) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				_locked = locked _veh;
				if(_veh in life_vehicles && player distance _veh < 8 && playerSide != west) then {
					if(_locked == 2) then {
						if(local _veh) then {
							_veh lock 0;
						} else {
