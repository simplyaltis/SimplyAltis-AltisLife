...
if(parseNumber(_amount) > life_dosh) exitWith {hint "You don't have that much to give!";ctrlShow[2001,true];};
if(isNull _unit) exitWith {ctrlShow[2001,true];};
...
life_dosh = life_dosh - (parseNumber(_amount));
[0] call SOCK_fnc_updatePartial;
[[_unit,_amount,player],"life_fnc_receiveMoney",_unit,false] spawn life_fnc_MP;
[[name player, getPlayerUID player, format["%1 gave $%3 to %2.", player getVariable["realname",name player], _unit getVariable["realname",name _unit], _amount]],"TON_fnc_logIt",false,false] spawn life_fnc_MP;
