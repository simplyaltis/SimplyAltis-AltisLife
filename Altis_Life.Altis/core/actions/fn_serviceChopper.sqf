...
private["_search","_ui","_progress","_cP","_pgText","_title","_cash"];
...
if(_cash < 1000) exitWith {hint localize "STR_Serive_Chopper_NotEnough"};

if(life_usedebit && __GETC__(life_donator) > 1) then
{
    life_rupees = life_rupees - 1000;
}
else
{
    life_dosh = life_dosh - 1000;
};
...
if(!alive (_search select 0) || (_search select 0) distance air_sp > 10) exitWith {life_action_inUse = false; hint localize "STR_Service_Chopper_Missing";_ui = "statusBar" call BIS_fnc_rscLayer;_ui cutRsc["statusBar","PLAIN"];};
