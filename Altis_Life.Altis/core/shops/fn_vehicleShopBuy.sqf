...
*/
private["_mode","_spawnPoints","_className","_basePrice","_colorIndex","_spawnPoint","_vehicle","_cash"];
...
_basePrice = round(_basePrice * life_donDis);
...
if(_cash < _basePrice) exitWith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - _cash] call life_fnc_numberText];};
...
if(life_usedebit && __GETC__(life_donator) > 1) then
{
    life_rupees = life_rupees - _basePrice;
}
else
{
    life_dosh = life_dosh - _basePrice;
};
playSound "buy";
