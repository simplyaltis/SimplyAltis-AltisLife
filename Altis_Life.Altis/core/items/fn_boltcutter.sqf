...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_safeEmpty","_nocops"];
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
    if((fed_bank getVariable["safe",-1]) < 1) then
    {
        _safeEmpty = true;
    }
    else
    {
        if({side _x == west} count playableUnits < 5) then
        {
            _nocops = true;
        }
        else
...
if(_safeEmpty) exitWith {hint localize "STR_Civ_VaultEmpty";};
if(_nocops) exitWith {hint localize "STR_Civ_NotEnoughCops";};
